...
docker run -d --privileged -e DOCKER_PASSWORD=${PASSWORD} -e LANG="en_US.UTF-8" -p 80:80 -p 11211:11211 --name django --link postgres:db --link existdb:xmldb --link fuseki:sparql --link celery:rabbitmq -v /docker:/docker:rw -t 0xffea/saucy-server-django
